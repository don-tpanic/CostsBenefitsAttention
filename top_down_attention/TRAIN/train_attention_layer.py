...
from TRAIN.utils.saving_utils import save_attention_weights
...
def train_model(wnid, index, model, 
    ...
    NAME = '{category}_run{run}_imp{imp}_float{float}'.format(category=description, run=run, imp=importance, float=float)
...
def main(attention_mode, patience, lr, importance, subsample_rate, imagenet_train, batch_size, epochs, float):
    ...
    for group in groupings:
        ...
        for i in range(len(group_wnids)):
            ...
            print(f'Learning attention weights for {description}, imp={importance}')
            print('============================================================')
            for run in ['98']:
                opt = Adam(lr=0.0003)
                if float == '16':
                    opt = tf.train.experimental.enable_mixed_precision_graph_rewrite(opt)
                model = AttentionModel_FilterWise(num_categories, attention_mode, lr, opt)
                ...
                save_attention_weights(model, attention_mode, description, importance, run, float)
                K.clear_session()
...
def execute():
    ...
    epochs = 500
    #####################
    importance = 1/999  # imp=1 is uniform attn (same as new notation alpha=0)
    subsample_rate = 0.1
    #####################
