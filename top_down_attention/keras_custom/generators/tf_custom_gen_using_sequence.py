...
class SafeDirectoryIterator(SafeIterator):
    ...
    def __init__(self,
        ...
        if self.focus_classes is None:
            for wnid in self.class_indices:
                # this is a way to speed up eval on negative example by not going thru all images
                subsample_weights[self.class_indices[wnid]] = 0.5
        else:
            for wnid in self.class_indices:
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        second_input = np.ones((batch_x.shape[0], SIZE), dtype=self.dtype)
